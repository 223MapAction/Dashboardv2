...
export const TaskModal = () => {
  ...
  useEffect(() => {
    if (taskSubmitAlert?.message && bodyRef.current) {
      bodyRef.current.scrollTo({ top: 0, behavior: 'smooth' });
    }
  }, [taskSubmitAlert?.message]);
    ...
    <>
      ...
      >
        ...
        <div className="am-offcanvas-body" ref={bodyRef}>
          {taskSubmitAlert && (
            <div className={`am-alert am-alert--${taskSubmitAlert.type === 'success' ? 'success' : 'danger'}`} role="alert" style={{ marginBottom: 'var(--spacing-4)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              {taskSubmitAlert.type === 'success' ? (
              ...
              <span className="am-alert__message" style={{ margin: 0 }}>{taskSubmitAlert.message}</span>
            </div>
          )}
                        ...
                        {taskAlert && (
                          <div className={`am-alert am-alert--${taskAlert.type === 'success' ? 'success' : 'danger'}`} role="alert" style={{ margin: 0, padding: '8px 12px', fontSize: 'var(--font-size-body-small)', borderRadius: 'var(--radius-md)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            {taskAlert.type === 'success' ? (
                              <TickCircle size={16} variant="Bold" color="var(--color-success)" style={{ flexShrink: 0 }} />
                            ) : (
